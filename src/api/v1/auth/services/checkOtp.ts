...
const checkOtp = async (otp: string, email: string): Promise<{ accessToken: string; refreshToken: string }> => {
    const user = await userModel.findOne({ email })
    if (!user) throw new Error('Không tồn tại email!')

    if (user.account.otpLockUntil && user.account.otpLockUntil > new Date()) {
        throw new Error('Bạn đã nhập sai OTP quá nhiều lần. Vui lòng thử lại sau 5 phút.')
    }
    ...
    if (user.account.otp !== otp) {
        ...
        if (user.account.otpAttempts >= MAX_OTP_ATTEMPTS) {
            ...
            await user.save()
            throw new Error('Bạn đã nhập sai OTP quá nhiều lần. Tài khoản bị khóa trong 5 phút.')
        }
