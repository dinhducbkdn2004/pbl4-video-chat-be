...
const validateLogin = [
    body('email').notEmpty().withMessage('Thiếu trường email').isEmail().withMessage('Email không hợp lệ'),
    body('password')
        .notEmpty()
...
const validateLoginByGoogle = [body('credential').notEmpty().withMessage('Thiếu trường credential')]
const validateCheckOtp = [
    body('otp').notEmpty().withMessage('thiếu trường otp').isLength({ min: 6 }).withMessage('Otp chưa đúng định dạng'),
    body('email').notEmpty().withMessage('thiếu trường email').isEmail().withMessage('Email chưa đúng định dạng')
]
